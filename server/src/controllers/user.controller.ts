...
export const getAssignableUsers = async (req: AuthRequest, res: Response) => {
  try {
    // Everyone who holds a platform LOGIN account and can own companies.
    // Jo kabhi login nahi kiya (lastLogin NULL) wo list me NAHI aayega —
    // dropdown me sirf "login kiya hua" staff dikhta hai.
    // SUPER_ADMIN is deliberately excluded so companies are never assigned
    // to super-admin accounts.
    const users = await prisma.user.findMany({
      where: { role: { in: ['EMPLOYEE', 'ADMIN'] }, status: 'ACTIVE', lastLogin: { not: null } },
...
export const createEmployee = async (req: AuthRequest, res: Response) => {
  try {
    ...
    // RBAC (HR Admin): admins can only create regular employee accounts.
...
export const updateEmployee = async (req: AuthRequest, res: Response) => {
  try {
    ...
    // RBAC (HR Admin): admins cannot modify Super Admin accounts or change user roles.
    if (req.user!.role !== 'SUPER_ADMIN' && employee.role === 'SUPER_ADMIN') {
      return res.status(403).json({ success: false, message: 'Admins cannot modify Super Admin accounts' });
    }
    if (req.user!.role === 'ADMIN' && role !== undefined && role !== employee.role) {
      return res.status(403).json({ success: false, message: 'Admins cannot change user roles' });
    }
...
export const changeEmployeePassword = async (req: AuthRequest, res: Response) => {
  try {
    ...
    // RBAC (HR Admin): admins cannot modify Super Admin accounts.
    if (req.user!.role !== 'SUPER_ADMIN' && employee.role === 'SUPER_ADMIN') {
      return res.status(403).json({ success: false, message: 'Admins cannot modify Super Admin accounts' });
    }
    const hashedPassword = await bcrypt.hash(password, 10);
...
export const deleteEmployee = async (req: AuthRequest, res: Response) => {
  try {
    ...
    // RBAC (HR Admin): admins cannot delete Super Admin accounts.
    if (req.user!.role !== 'SUPER_ADMIN' && employee.role === 'SUPER_ADMIN') {
      return res.status(403).json({ success: false, message: 'Admins cannot modify Super Admin accounts' });
    }
