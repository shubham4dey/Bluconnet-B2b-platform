...
import { Eye, EyeOff, Mail, Lock, UserPlus, Building2, BarChart3, ShieldCheck, Loader2 } from 'lucide-react';
        ...
        <div className="w-full max-w-md animate-slide-up">
          ...
          <div className="rounded-2xl border border-slate-200/60 bg-white p-8 shadow-card sm:p-10">
            <div className="mb-8">
              ...
              <div className="mb-5 flex gap-1 rounded-xl bg-slate-100 p-1" role="tablist" aria-label="Authentication mode">
                <button
                  type="button"
                  role="tab"
                  aria-selected={mode === 'signin'}
                  onClick={() => { setMode('signin'); setError(''); setLoading(false); }}
                  className={`flex flex-1 items-center justify-center rounded-lg px-4 py-2 text-sm font-semibold transition-colors ${
                    mode === 'signin' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:bg-slate-200 hover:text-slate-700'
                  }`}
                >
                  Sign in
                </button>
                <button
                  ...
                  className={`flex flex-1 items-center justify-center rounded-lg px-4 py-2 text-sm font-semibold transition-colors ${
                    mode === 'signup' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:bg-slate-200 hover:text-slate-700'
                  }`}
                >
                  Sign up
                </button>
              ...
              <p className="mt-1.5 text-sm text-slate-500">
                {mode === 'signup' ? 'Sign up to join the workspace.' : 'Sign in to your account to continue.'}
              </p>
            </div>
            ...
            <p className="mt-6 text-center text-xs text-slate-400">
              {mode === 'signup'
                ? 'Passwords must be at least 6 characters. New accounts are created as Employees.'
